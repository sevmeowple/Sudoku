<script lang="ts">
    ...
    function setCellValue(value: number) {
        if (selectedCell.row !== -1 && selectedCell.col !== -1) {
            const currentCellValue = userBoard[selectedCell.row][selectedCell.col];
            ...
            if (value === 0) {
                ...
                if (currentCellValue !== 0 && currentCellValue !== solvedBoard[selectedCell.row][selectedCell.col]) {
                    falseNum = Math.max(0, falseNum - 1); // 减少错误计数
                }
            } else if (value === solvedBoard[selectedCell.row][selectedCell.col]) {
                // 正确填写
                if (cellElement) {
                    cellElement.classList.remove('incorrect');
                    ...
                    >
                        ...
                        {#if selectedCell.row === rowIndex && selectedCell.col === colIndex}
                            <div class="number-selector">
                                ...
                                {/each}
                                <div class="clear-button" on:click={() => setCellValue(0)}>清除</div>
                            </div>
                        {/if}
                    </td>
                {/each}
